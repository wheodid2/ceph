...
#ifndef OSD_OWNER_INFO_H_
#define OSD_OWNER_INFO_H_
...
using OSDReqRef = MClientRequest::const_ref;
...
using OwnerId = ClientId;
using SessionId = std::string;
using OwnerCnt = double;
using GVF = double;

using Queue = crimson::dmclock::PushPriorityQueue<OwnerId, ClientRequest>;
...
private:
  ...
  OwnerId owner_id;
public:
  Request(RequestType _type, OwnerId _owner_id) :
    type(_type), owner_id(_owner_id) {};

  Request(RequestType _type, OwnerId _owner_id, RequestCB _cb_func) :
    type(_type), owner_id(_owner_id), cb_func(_cb_func) {};
  ...
  const OwnerId& get_owner_id() const
  {
    return owner_id;
  }
  RequestCB cb_func;
...
public:
  const OSDReqRef osd_req_ref;
  Time time;
  uint32_t cost;
  explicit ClientRequest(const OSDReqRef &_osd_req_ref, OwnerId _id,
      double _time, uint32_t _cost) :
      Request(RequestType::CLIENT_REQUEST, _id),
      osd_req_ref(_osd_req_ref), time(_time), cost(_cost) {};
};

class UpdateRequest : public Request {
public:
  UpdateRequest(OwnerId _id):
    Request(RequestType::UPDATE_REQUEST, _id) {};
  UpdateRequest(OwnerId _id, RequestCB _cb_func):
...
class OwnerInfo : public QoSInfo {
private:
  bool use_default;
  std::set<SessionId> session_list;
  int inflight_requests;
  OwnerCnt owner_count;
  GVF global_view_factor;

public:
  explicit OwnerInfo():
    QoSInfo(0.0, 0.0, 0.0), use_default(true), inflight_requests(0), owner_count(0.0), global_view_factor(0.0)  {};
  ...
  OwnerCnt get_owner_count() const
  {
    return owner_count;
  }

  void increase_owner_count()
  {
    owner_count++;
  }

  void reset_owner_count()
  {
    owner_count = 0.0;
  }
  ...
  void dump(Formatter *f, const std::string &oid) const
  {
    f->dump_string("owner_id", oid);
...
ostream& operator<<(ostream& os, const OwnerInfo* vi);

class osd_dmclock_conf : public QoSInfo {
private:
  bool enabled;

public:
  osd_dmclock_conf(): QoSInfo(0.0, 0.0, 0.0), enabled(false){};
...
private:
  ...
  osd_dmclock_conf default_conf;
  int total_inflight_requests;
  MDSRank *mds;
  Queue *dmclock_queue;
  std::map<OwnerId, OwnerInfo> owner_info_map;
  mutable std::mutex owner_info_lock;
...
public:
  ...
  std::map<OwnerId, OwnerInfo> &get_owner_info_map()
  {
    return owner_info_map;
  }
  osd_dmclock_conf get_default_conf()
  {
    return default_conf;
  }

  // owner QoS info management
  void create_owner_info(const OwnerId &oid, const ClientInfo &client_info, const bool use_default);
  void add_session_to_owner_info(const OwnerId &oid, const SessionId &sid);
  void update_owner_info(const OwnerId &oid, const ClientInfo& client_info, const bool use_default);
  OwnerInfo *get_owner_info_ptr(const OwnerId &oid);
  bool copy_owner_info(const OwnerId &oid, OwnerInfo &vi);
  bool check_owner_info_existence(const OwnerId &oid);
  void delete_session_from_owner_info(const OwnerId &oid, const SessionId &sid);
  void set_default_owner_info(const OwnerId &oid);
  ...
  void broadcast_qos_info_update_to_osd(const OwnerId& oid, const dmclock_info_t &dmclock_info);

  void broadcast_to_worker_for_owner_cnt();
  void broadcast_from_ctrler_to_worker(std::map<mds_rank_t,std::map<OwnerId,GVF>> gvf_map_per_mds);
  void lonely_sending_to_ctrler(const std::map<OwnerId, OwnerCnt>& volcnt_map);
  ...
  void handle_mds_request(const OSDReqRef &req);
  template<typename R>
  void enqueue_client_request(const R &mds_req, OwnerId owner_id);
  void submit_request_to_mds(const OwnerId &, std::unique_ptr<ClientRequest> &&, const PhaseType&, const uint64_t);
  const ClientInfo *get_client_info(const OwnerId &oid);
  ...
  CInode *read_xattrs(const OwnerId oid);
  ...
  void enqueue_update_request(const OwnerId& oid);
  void enqueue_update_request(const OwnerId& oid, RequestCB cb_func);
  uint32_t get_request_queue_size() const;
 
  int active_mds_num;
  std::deque<std::pair<mds_rank_t,std::map<OwnerId,OwnerCnt>>> volcnt_queue;

  const OwnerId get_owner_id(Session *session);
  const SessionId get_session_id(Session *session);
  const OwnerId convert_subvol_root(const OwnerId& owner_id);
  ...
  void increase_inflight_request(const OwnerId &oid);
  void decrease_inflight_request(const OwnerId &oid);
  int get_inflight_request(const OwnerId &oid);

  void increase_owner_count(const OwnerId &oid);
  void reset_owner_count(const OwnerId &oid);
  OwnerCnt get_owner_count(const OwnerId &oid);

  void set_global_view_factor(const OwnerId &oid, const GVF &gvf);
  GVF get_global_view_factor(const OwnerId &oid);

  void shutdown();
  friend ostream& operator<<(ostream& os, const OwnerInfo* vi);
